...
fn list_communities_on_empty_community_config_returns_empty() {
    ...
    let list = <ApplicationFacade<_> as CommunityUseCase>::list_communities(&facade).unwrap();
...
fn create_community_writes_to_disk() {
    ...
    let result = <ApplicationFacade<_> as CommunityUseCase>::create_community(&facade, community);
    assert!(result.is_ok());
    let list = <ApplicationFacade<_> as CommunityUseCase>::list_communities(&facade).unwrap();
