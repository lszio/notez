...
fn agenda_on_empty_projection_is_empty() {
    ...
    let view = <Engine<_> as TaskUseCase>::agenda(&facade).unwrap();
...
fn para_overview_on_empty_projection_has_four_empty_buckets() {
    let (_dir, facade) = make_facade();
    let view = <Engine<_> as TaskUseCase>::para_overview(&facade).unwrap();
...
fn transition_task_on_missing_resource_returns_not_found() {
    ...
    let err = <Engine<_> as TaskUseCase>::transition_task(
        &mut facade,
        &r_ref,
        "DONE",
        "2026-08-02",
