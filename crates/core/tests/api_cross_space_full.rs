...
//! equal across spaces while `ref` and `source_id` differ.
...
fn cross_space_scan_then_aggregate_by_object_id() {
    ...
    // object_id so we can query for it in both spaces.
    let doc_a = scanned_a
        .resources
        .iter()
    ...
    // object_id matches across spaces (the cross-Space identity claim).
