use document::security::{SecurityError, SecurityGuard};
use std::path::Path;
...
fn security_guard_prevents_path_traversal_and_oversized_files() {
    ...
    assert!(SecurityGuard::sanitize_path(space_root, valid_path).is_ok());

    let invalid_path = Path::new("/space/../../etc/passwd");
    assert!(SecurityGuard::sanitize_path(space_root, invalid_path).is_err());
