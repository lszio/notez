use domain::{Resource, ResourceKind, ResourceRef, ResourceRelation, Selector};
use storage::{ProjectionStore, SqliteProjection};
...
fn projection_replace_get_query_clear() {
    ...
    let res1 = sample_resource(ResourceKind::Document, "Sync Design", "01J00000000000000000000001", "native");
    let res2 = sample_resource(ResourceKind::Heading, "Architecture Sync", "01J00000000000000000000002", "native");
    ...
    store.replace_source("native", vec![res1.clone(), res2.clone()], vec![rel1]).unwrap();

    let fetched = store.get(&res1.r#ref).unwrap().expect("resource 1 should exist");
    ...
    store.replace_source("native", vec![res1.clone()], vec![]).unwrap();
...
fn projection_rollback_on_duplicate_refs() {
    let mut store = SqliteProjection::in_memory().unwrap();
    let res1 = sample_resource(ResourceKind::Document, "Doc 1", "01J00000000000000000000001", "native");
    let res2_dup = sample_resource(ResourceKind::Document, "Doc 2", "01J00000000000000000000001", "native");
...
fn projection_disk_persistence() {
    ...
    let res1 = sample_resource(ResourceKind::Document, "Persistent Doc", "01J00000000000000000000005", "native");
    ...
    let fetched = store_reopened.get(&res1.r#ref).unwrap().expect("should persist");
    assert_eq!(fetched, res1);
}
