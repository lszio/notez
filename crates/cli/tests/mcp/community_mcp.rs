...
async fn mcp_community_create_derive_and_export_skill() {
    ...
    let requests = vec![
        ...
        json!({"jsonrpc": "2.0", "id": 2, "method": "tools/call", "params": {"name": "community_create", "arguments": {"space": source_root.to_string_lossy(), "id": "mcp_comm", "name": "MCP Comm", "title_contains": "Sync"}}}).to_string(),
        json!({"jsonrpc": "2.0", "id": 3, "method": "tools/call", "params": {"name": "derive_artifact", "arguments": {"space": source_root.to_string_lossy(), "community": "mcp_comm", "recipe": "summary"}}}).to_string(),
        json!({"jsonrpc": "2.0", "id": 4, "method": "tools/call", "params": {"name": "export_skill", "arguments": {"space": source_root.to_string_lossy(), "community": "mcp_comm", "description": "MCP Exported Skill", "out": skill_out.to_string_lossy()}}}).to_string(),
    ];
