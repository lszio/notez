...
fn main() {
    ...
    match cli.command {
        ...
        Commands::Watch(args) => {
            run_watch(args, &r_config.space_root);
        }
    }
}

/// Run the `notez watch` subcommand.
fn run_watch(args: commands::WatchArgs, space_root: &std::path::Path) {
    use notez_core::application::WatchService;
    let svc = WatchService::new();
    match args.command {
        None | Some(commands::WatchCommands::Start) => {
            if let Err(e) = svc.start(space_root) {
                eprintln!("watch start failed: {e}");
                std::process::exit(2);
            }
            eprintln!("watching {} (Ctrl-C to stop)", space_root.display());
            // Poll events every 500ms; SIGINT (Ctrl-C) ends the loop.
            // We track how many events we have already printed to
            // avoid re-emitting the same event.
            let running = std::sync::Arc::new(std::sync::atomic::AtomicBool::new(true));
            install_sigint_handler(running.clone());
            let mut printed: usize = 0;
            while running.load(std::sync::atomic::Ordering::SeqCst) {
                std::thread::sleep(std::time::Duration::from_millis(500));
                let events = svc.events(space_root, 200);
        ...
        Some(commands::WatchCommands::Stop) => {
            if svc.stop(space_root) {
                println!("watch stopped.");
            } else {
                eprintln!("no watch was active for {}", space_root.display());
            }
        }
        Some(commands::WatchCommands::Status { limit }) => {
            let status = svc.status(space_root);
            let events = svc.events(space_root, limit);
            match status {
                None => println!("no watch is active for {}", space_root.display()),
                Some(s) => println!(
                    "watching {} since epoch+{}s ({} events buffered)",
                    s.space_root.display(),
                    s.started_at
                        .duration_since(std::time::UNIX_EPOCH)
                        .map(|d| d.as_secs())
                        .unwrap_or(0),
