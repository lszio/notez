...
//!   - a [`SendService`] handle wrapping the `SqliteProjection`,
//!   - an `Arc` over [`PreviewerCatalog`] for resolving and rendering previews.
...
impl WebState {
    ...
    ///
    /// Opens (or creates) the SQLite index at
    /// `<space_root>/.notez/index.sqlite`, wraps it in a `SendService`
    /// so handlers can call into it from any axum task, and registers
    /// the default 14-previewers catalog.
    pub fn from_space(root: &Path) -> anyhow::Result<Self> {
        ...
        let service = SendService::new(store);
