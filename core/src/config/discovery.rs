use crate::config::model::{ConfigError, GlobalConfig, SpaceConfig, SpaceRegistration};
    ...
    match selector {
        SpaceSelector::Name(name) => {
            ...
            let space_config = if space_toml.exists() {
                ...
                SpaceConfig {
                    version: 1,
                    space: crate::config::model::SpaceIdentity { name: name.to_string(), database: crate::config::model::default_database() },
...
space_config: space_config.clone(),
        ...
        SpaceSelector::Path(p) => {
            ...
            let cfg = if space_toml.exists() {
                ...
                SpaceConfig {
                    version: 1,
                    space: crate::config::model::SpaceIdentity { name: root.file_name().unwrap_or_default().to_string_lossy().to_string(), database: crate::config::model::default_database() },
                    workflow: Default::default(),
                    sources: Vec::new(),
                    link_overrides: serde_json::Value::Null,
                }
